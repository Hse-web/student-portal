...
require_role('admin');
require_once __DIR__ . '/../includes/functions.php';   // defines create_notification(), set_flash(), get_flash()
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        // If applied right away, notify that one student
        if ($is_applied) {
            // look up the group label
            $lbl = $conn
                ->prepare("SELECT label FROM art_groups WHERE id=? LIMIT 1");
    ...
    if ($action === 'delete') {
        ...
        // fetch student_id for redirect
        $tmp = $conn
            ->prepare("SELECT student_id FROM student_promotions WHERE id=? LIMIT 1");
...
// ─────────────────────────────────────────────────────────────────────────────
// 2) Fetch all students & art groups for dropdowns
// ─────────────────────────────────────────────────────────────────────────────
$students = $conn
...
// ─────────────────────────────────────────────────────────────────────────────
// 3) Which student are we viewing?
// ─────────────────────────────────────────────────────────────────────────────
$filterSid = (int) ($_GET['student_id']
              ?? ($students[0]['id'] ?? 0));
