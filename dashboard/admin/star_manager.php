...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        ...
        if ($rid < 1) {
            ...
            try {
                ...
                // 2) Ensure stars row
                $stmt = $conn->prepare("
                  SELECT star_count FROM stars WHERE student_id=?
                ");
                ...
                if ($currentBalance < $stars_required) {
                    throw new Exception('Insufficient stars.');
                }
                ...
                create_notification(
                  ...
                  [$student_id],
                  'Redemption Approved',
                  "Your redemption request #{$rid} has been approved and {$stars_required} stars deducted."
                );
            } catch (Exception $e) {
                $conn->rollback();
    ...
    if ($action === 'reject') {
        ...
        if ($rid < 1) {
            ...
            // fetch student for notify
            $row = $conn->query("SELECT student_id FROM star_redemptions WHERE id={$rid}")
                        ->fetch_assoc();
            ...
            if ($student_id) {
              create_notification(
                ...
                [$student_id],
                'Redemption Rejected',
                "Your redemption request #{$rid} has been rejected."
              );
            }
        }
        header('Location: ?page=star_manager');
    ...
    if ($action === 'award') {
        ...
        if ($stu<1 || $amt<1 || $reason==='') {
            ...
            try {
                ...
                // award
                $stmt = $conn->prepare("
                  UPDATE stars SET star_count = star_count + ? WHERE student_id=?
                ");
                ...
                create_notification(
                  ...
                  [$stu],
                  'Stars Awarded',
                  "You have been awarded {$amt} stars. Reason: {$reason}"
                );
            } catch (Exception $e) {
                $conn->rollback();
                $_SESSION['flash'] = ['type'=>'danger','msg'=>'Error: '.$e->getMessage()];
...
$stmt = $conn->prepare("
  SELECT r.id, r.student_id, st.name AS student_name,
         r.reward_title, r.stars_required, r.requested_at
      ...
      <div class="card-body">
        ...
        <form method="POST" class="row g-2 align-items-end">
          ...
          <div class="col-md-3">
            ...
            <input type="number" name="award_amount" min="1" class="form-control form-control-sm" placeholder="e.g. 10" required>
          </div>

          <div class="col-md-4">
            <label class="form-label">Reason</label>
            <input type="text" name="award_reason" class="form-control form-control-sm" placeholder="e.g. Excellent attendance" required>
          </div>
        ...
        <?php else: ?>
          <div class="table-responsive">
            <table class="table table-striped align-middle">
              <thead><tr>
                <th>Req ID</th><th>Student</th><th>Reward</th>
                <th>Stars Req'd</th><th>Requested At</th><th>Actions</th>
              </tr></thead>
              <tbody>
