...
// 1) Bootstrap session, CSRF, and auth helpers
...
$menu = [
    ...
    'delete_student'      => ['bi-trash','Delete Student'],
...
// 4) Fetch logged-in admin username
...
// 5) Pre-handle POST actions for some pages
...
// 6) For dashboard cards, count totals
$tableMap = ['students'=>'students'];
$counts   = [];
foreach ($tableMap as $key => $tbl) {
    $row = $conn->query("SELECT COUNT(*) AS cnt FROM `$tbl`")
                ->fetch_assoc();
