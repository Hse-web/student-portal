...
$plansList  =$conn->query("SELECT id, CONCAT(plan_name,' (',duration_months,'m)') AS label FROM payment_plans GROUP BY id ORDER BY plan_name")->fetch_all(MYSQLI_ASSOC);
?>
...
<div class="max-w-7xl mx-auto p-6 space-y-6 bg-white rounded-lg shadow">
  ...
  <?php if ($flash): ?>
    <div id="toastMsg" class="fixed top-4 right-4 z-50 bg-<?= $flash['type']==='danger'?'red':'green' ?>-600 text-white px-4 py-2 rounded shadow">
  ...
  <form method="get" class="grid grid-cols-1 md:grid-cols-4 gap-4">
    ...
    <input type="text" name="search" placeholder="Search name, email or group…" value="<?=htmlspecialchars($search)?>" class="border p-2 rounded">
    ...
    <select name="plan" class="border p-2 rounded">
      ...
      <?php foreach($plansList as $p): ?>
        <option value="<?=$p['id']?>" <?=$plan===$p['id']?'selected':''?>><?=htmlspecialchars($p['label'])?></option>
      <?php endforeach; ?>
    </select>
  ...
  <form id="bulkDeleteForm" method="post" action="?page=delete_bulk">
    ...
    <div class="overflow-x-auto">
      <table class="w-full table-auto bg-white divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-100">
          <?php if(!$students): ?>
            <tr><td colspan="9" class="p-4 text-center text-gray-500">No students found.</td></tr>
          <?php else: foreach($students as $s): ?>
            <tr>
              <td class="px-4 py-2"><input type="checkbox" name="student_ids[]" value="<?=$s['id']?>"></td>
              ...
              <td class="px-4 py-2 flex flex-wrap gap-2">
                <a href="?page=edit_student&id=<?=$s['id']?>" class="px-2 py-1 border border-purple-600 text-purple-600 rounded hover:bg-purple-600 hover:text-white">Edit</a>
                <button type="button" onclick="confirmIndividualDelete(<?=$s['id']?>)" class="px-2 py-1 border border-red-600 text-red-600 rounded hover:bg-red-600 hover:text-white">Delete</button>
              </td>
            </tr>
    ...
    <?php if($students): ?>
      <div class="mt-4 flex justify-end">
        <button type="button" onclick="confirmBulkDelete()" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">🗑 Delete Selected</button>
      </div>
    <?php endif; ?>
  </form>
  ...
  <?php if($totalPages>1): ?>
    <nav class="flex justify-center mt-6">
      <ul class="inline-flex -space-x-px">
        <li><a href="?<?=http_build_query(array_merge($_GET,['p'=>max(1,$pageNo-1)]))?>" class="px-3 py-1 border rounded-l">Prev</a></li>
        <?php for($i=1;$i<=$totalPages;$i++): ?>
          <li><a href="?<?=http_build_query(array_merge($_GET,['p'=>$i]))?>" class="px-3 py-1 border <?= $i===$pageNo?'bg-purple-600 text-white':'hover:bg-gray-100' ?>"><?=$i?></a></li>
        <?php endfor; ?>
        <li><a href="?<?=http_build_query(array_merge($_GET,['p'=>min($totalPages,$pageNo+1)]))?>" class="px-3 py-1 border rounded-r">Next</a></li>
      </ul>
    </nav>
  <?php endif; ?>
</div>
...
<div id="confirmModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden items-center justify-center">
  <div class="bg-white rounded-lg p-6 shadow max-w-sm w-full">
    ...
    <div class="flex justify-end gap-2">
      ...
      <button onclick="submitBulkDelete()" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Delete</button>
    </div>
  </div>
</div>

<!-- ✅ Individual Confirmation Modal -->
<form method="post" action="?page=delete_student" id="individualDeleteForm">
  <input type="hidden" name="csrf_token" value="<?=$csrf?>">
