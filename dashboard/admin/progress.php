...
if (isset($_GET['edit'])) {
  ...
  if ($eid > 0) {
    ...
    $stmt->execute();
    $edit = $stmt->get_result()->fetch_assoc() ?: ['id'=>0,'student_id'=>0,'month'=>''];
    $stmt->close();
  } else {
    $edit = ['id'=>0,'student_id'=>0,'month'=>''];
  }
}
...
// 7a) Generate last 12 months for Add/Edit
$monthOptions = [];
$dt = new DateTime('first day of this month');
for ($i = 0; $i < 12; $i++) {
    $monthOptions[] = $dt->format('Y-m');
    $dt->modify('-1 month');
}

// 7b) Fetch distinct months for Filter
$filterOptions = [];
$res = $conn->query("
...
// 8) Fetch paginated list with a LEFT JOIN
$cols = 'p.id,p.month, COALESCE(s.name,\'Unknown Student\') AS student_name';
foreach ($categories as $key => $_) {
...
// 9) Fetch student list for Add/Edit
...
<body class="bg-light">
  <div class="container py-5">
    ...
    <?php if($edit !== null): ?>
      <div class="card mb-5">
        <div class="card-body">
          <h5 class="card-title"><?= ($edit['id'] ?? 0) ? 'Edit' : 'Add' ?> Progress</h5>
          <?php if($errors): ?>
          ...
          <form method="POST">
            ...
            <?php foreach($categories as $key=>$label): ?>
              <div class="row mb-3 align-items-end">
                <div class="col-md-3">
                  ...
                  <select name="<?= $key ?>" data-cat="<?= $key ?>" class="form-select metric-grade" required>
                    <option value="">—</option>
            ...
            <button class="btn btn-success"><?= ($edit['id'] ?? 0) ? 'Save Changes' : 'Submit' ?></button>
    ...
    <form method="GET" class="row g-3 mb-4">
      ...
      <div class="col-md-3">
        ...
        <select name="month" class="form-select">
          ...
          <?php foreach($filterOptions as $m): ?>
      ...
      <table class="table table-striped">
        <thead class="table-light">
          <tr>
            <th>Month</th>
            <th>Student</th>
            <?php foreach($categories as $label): ?><th><?= $label ?></th><?php endforeach; ?>
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
            ...
            <?php foreach($rows as $r): ?>
              <tr>
                ...
                <?php foreach(array_keys($categories) as $key): ?>
                  <td>
                    <?= $grades[$r[$key]] ?? '—' ?><br>
                    <small class="text-muted"><?= htmlspecialchars($r[$remarkCols[$key]]) ?></small>
                  </td>
                <?php endforeach; ?>
                <td>
    ...
    document.addEventListener('DOMContentLoaded', ()=>{
      document.querySelectorAll('.metric-grade').forEach(sel=>{
        sel.addEventListener('change', ()=>{
          ...
          const bucket = (window.remarkTemplates[cat]||{})[sel.value]||[];
          if (!bucket.length) return;
          document.querySelector(`.metric-remark[data-cat="${cat}"]`).value =
