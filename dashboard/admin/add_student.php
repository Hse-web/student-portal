...
$plans     = $conn->query("
  SELECT p.id, p.centre_id, p.art_group_id,
         CONCAT(p.plan_name,' (',p.duration_months,'m) — ₹',p.amount) AS label
    FROM payment_plans p
   ORDER BY p.centre_id, p.art_group_id, p.duration_months
  ...
  if (empty($errors)) {
    ...
    try {
      ...
      $s = $conn->prepare("
        ...
        (user_id, centre_id, name, email, phone, gender, group_name,
...
// d2) Log referral entry
if ($refId) {
  $stmt = $conn->prepare("
    INSERT INTO referrals
      (referrer_student_id, referee_student_id,
       referrer_discount_pct, referee_discount_pct,
       created_at, referrer_applied, referee_applied)
    VALUES (?, ?, 10.00, 10.00, NOW(), 0, 0)
  ");
  $stmt->bind_param('ii', $refId, $stuId);
  $stmt->execute();
  $stmt->close();
}
     ...
     // f) Payment due — seed with due_date so student can upload proof immediately
$amt = calculate_student_fee($conn, $stuId, $plan_id, true, false)['total'];
$dueDate = date('Y-m-05'); // current month 5th as first due date
$pay = $conn->prepare("
  INSERT INTO payments (student_id, status, amount_paid, amount_due, due_date)
  VALUES (?, 'Pending', 0.00, ?, ?)
");
$pay->bind_param('ids', $stuId, $amt, $dueDate);
$pay->execute();
$pay->close();
  ...
  <div class="bg-white rounded-lg shadow p-6">
    ...
    <form method="post" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div class="sm:col-span-2">
        ...
        <select name="initial_group_id" required class="w-full border rounded px-3 py-2">
          <option value="">— Select Stage —</option>
          <?php foreach ($artGroups as $g): ?>
            <option value="<?= $g['id'] ?>" <?= $form['initial'] === $g['id'] ? 'selected' : '' ?>>
      ...
      <div>
        ...
        <select name="centre_id" id="centreSelect" required class="w-full border rounded px-3 py-2">
      ...
      <div>
        ...
        <select name="plan_id" id="planSelect" required class="w-full border rounded px-3 py-2">
          <option value="">— Plan —</option>
          <?php foreach ($plans as $p): ?>
            <option value="<?= $p['id'] ?>"
              data-centre-id="<?= $p['centre_id'] ?>"
              data-art-group="<?= $p['art_group_id'] ?>"
      ...
      <div class="sm:col-span-2">
        ...
        <input type="email" name="referrer" class="w-full border rounded px-3 py-2"
               value="<?= $form['referrer'] ?>">
      ...
      <div class="sm:col-span-2 text-right space-x-4">
        ...
        <a href="index.php?page=students" class="text-gray-700 hover:underline">Cancel</a>
      </div>
    </form>
  </div>
</div>
...
<script>
  ...
  const stageSelect  = document.querySelector('select[name=initial_group_id]');
  const planSelect   = document.getElementById('planSelect');

  function filterPlans() {
    const cid = centreSelect.value;
    const gid = stageSelect.value;
    planSelect.querySelectorAll('option').forEach(opt => {
      if (!opt.value) return;
      const matchCentre = opt.dataset.centreId === cid;
      const matchGroup  = opt.dataset.artGroup === gid;
      opt.style.display = (matchCentre && matchGroup) ? '' : 'none';
    });

    // Optional: auto-select if a visible option matches
    const firstVisible = [...planSelect.options].find(opt => opt.style.display === '');
    if (firstVisible) planSelect.value = firstVisible.value;
  }

  centreSelect.addEventListener('change', filterPlans);
  stageSelect.addEventListener('change', filterPlans);
