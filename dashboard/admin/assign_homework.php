...
require_role('admin');
require_once __DIR__ . '/../includes/functions.php';   // must define create_notification()
...
$centreGroups = [];
$res = $conn->query("
    SELECT s.centre_id, ag.id AS art_group_id, ag.label
      FROM students s
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $stmt->bind_param('i', $aid);
        $stmt->execute();
        if ($stmt->affected_rows && function_exists('log_audit')) {
            log_audit(
                $conn,
                $_SESSION['user_id'],
                'DELETE',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'assign') {
    ...
    if (empty($errors)) {
        ...
        if (empty($ids)) {
            ...
            // ─── Notify students ─────────────────
            $msg = "You have a new homework assignment:\n“{$title}”\n{$description}";
            create_notification($conn, $ids, 'New Homework Assigned', $msg);
        }
    }
}
      ...
      <form method="post" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
          ...
          <textarea name="description" rows="4" required
                    class="w-full border-gray-300 rounded-lg p-2"><?= htmlspecialchars($_POST['description'] ?? '', ENT_QUOTES) ?></textarea>
        </div>

        <div>
          <label class="block text-gray-700 mb-1">Attachment (PDF/JPG/PNG)</label>
          <input type="file" name="attachment" accept=".pdf,.jpeg,.jpg,.png"
                 class="w-full border-gray-300 rounded-lg p-2">
        </div>

        <button type="submit"
                class="bg-purple-600 hover:bg-purple-700 text-white py-3 px-6 rounded-lg w-full md:w-auto">
          Assign to Students
        </button>
      </form>
    </div>
      ...
      <table class="min-w-full divide-y divide-gray-200 text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <?php if (empty($assignments)): ?>
            <tr>
              <td colspan="6" class="p-4 text-center text-gray-500">No assignments yet.</td>
            </tr>
          <?php else: foreach ($assignments as $a): ?>
            <tr class="hover:bg-gray-50">
              <td class="p-3"><?= htmlspecialchars($a['date']) ?></td>
              <td class="p-3"><?= htmlspecialchars($a['centre']) ?></td>
              <td class="p-3"><?= htmlspecialchars($a['stage']) ?></td>
              <td class="p-3"><?= htmlspecialchars($a['title']) ?></td>
              <td class="p-3 text-center"><?= (int)$a['cnt'] ?></td>
              <td class="p-3 text-center">
                <form method="post" onsubmit="return confirm('Delete this batch?')" class="inline">
                  <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
