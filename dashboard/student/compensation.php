...
// 4) Absences in *entire* subscription window ----------------------------
$absentDates = [];
if (! $error) {
    $stmt = $conn->prepare("
        SELECT date
         ...
         ORDER BY date
    ");
    $stmt->bind_param('iss', $student_id, $subStart, $subEnd);
    $stmt->execute();
    $res = $stmt->get_result();
...
<div class="container my-4">
  ...
  <?php if (empty($absentDates) && ! $error): ?>
    <div class="alert alert-info">No un-compensated absences in your subscription period.</div>
  <?php elseif (! $error): ?>
    <form method="POST" class="card card-body w-100 w-md-50">
      <div class="mb-3">
        <label class="form-label">Which absence?</label>
        <select name="absent_date" class="form-select" required>
          <option value="">— pick a date —</option>
          <?php
            /* group absences by YYYY-MM for easier navigation */
            $byMonth = [];
          ...
          ?>
            <optgroup label="<?= htmlspecialchars($month) ?>">
