...
$studentMenu = [
  ...
  'logout'         => ['bi-box-arrow-right','Logout',           '../../logout.php'],
];
?><!DOCTYPE html>
<html lang="en">
<head>
...
<body class="flex flex-col min-h-screen bg-gray-100">
  ...
  <div id="mobileMenu" class="hidden md:hidden bg-student-secondary text-gray-900 shadow-lg">
    <ul class="flex flex-col divide-y divide-gray-200">
      ...
      ?>
        <li>
          <a href="<?= $href ?>" class="flex items-center p-3 <?= $active ?>">
    ...
    <div class="flex-1 flex flex-col">
      ...
      <main class="flex-1 overflow-auto p-6">
        <?php if ($page==='dashboard'): ?>
          ...
          <?php
          // ── Build Art Journey data ───────────────────────────────────────

          // fetch current and next art_group_id
          ...
          <!-- Responsive Card-Based Art Journey -->
          <div class="bg-white p-6 rounded-2xl shadow-xl mb-8 relative overflow-visible">
            ...
            <svg class="absolute inset-0 w-full h-full" viewBox="0 0 1000 200" preserveAspectRatio="none" aria-hidden="true">
              <path d="M100,150 C250,0 400,200 550,50 S800,200 900,100" stroke="#E5E7EB" stroke-width="4" fill="none"/>
            </svg>
            ...
            <div class="relative z-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 px-4">
              ...
              ?>
                <div class="p-4 rounded-xl border-2 <?= $bg ?> <?= $border ?> flex flex-col items-center text-center shadow-sm hover:shadow-md transition">
                  <div class="text-4xl"><?= $emoji ?></div>
                  <div class="mt-3 font-medium text-lg"><?= htmlspecialchars($step['label']) ?></div>
                  <div class="mt-2 px-2 py-1 bg-<?= $clr ?>-100 text-<?= $clr ?>-800 rounded-full text-xs font-semibold">
                    <?= $badgeText ?>
                  </div>
                </div>
              <?php endforeach; ?>
