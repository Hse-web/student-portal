...
// Included via index.php?page=compensation (so we inherit your sidebar/header).
// ───────────────────────────────────────────────────────────────────────────────

if (basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'])) {
  // Prevent direct URL hits—always go through the wrapper:
  echo '<script>
        window.location.href = "/student-portal/dashboard/student/index.php?page=compensation";
      </script>';
  exit;
}
...
if (!$stmt->fetch()) {
  ...
  header('Location: /student-portal/dashboard/student/index.php');
  exit;
}
$stmt->close();

// 2) Pull + clear any flashes
$ok     = $_SESSION['flash']       ?? '';
$error  = $_SESSION['flash_error'] ?? '';
unset($_SESSION['flash'], $_SESSION['flash_error']);

// ───────────────────────────────────────────────────────────────────────
// Centre A/B: show Video Compensation only, then exit
// ───────────────────────────────────────────────────────────────────────
if ($centreId !== 3) {
  // a) this month absences
  $month = date('Y-m');
  $stmt = $conn->prepare("
    SELECT date
      FROM attendance
  ...
  // b) how many video comps watched
  $stmt = $conn->prepare("
    SELECT COUNT(*)
      FROM attendance
     WHERE student_id = ?
  ...
  <div class="container my-4">
    ...
    <?php else: ?>
      <div class="list-group">
        <?php
        // fetch all watched dates this month
        $stmt2 = $conn->prepare("
          SELECT date
            FROM attendance
           WHERE student_id = ?
             ...
             AND DATE_FORMAT(date,'%Y-%m') = ?
        ");
        $stmt2->bind_param('is', $student_id, $month);
        $stmt2->execute();
        $r2 = $stmt2->get_result();
        $watchedDates = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
  ...
  if (!$error) {
    ...
    echo '<script>window.location.href = "/student-portal/dashboard/student/index.php?page=compensation";</script>';
    exit;
  }
}
