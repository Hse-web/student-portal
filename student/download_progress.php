...
// 5) Fetch the latest single progress row in that span
$stmt = $conn->prepare("
  SELECT month, hand_control, hc_remark,
...
// 5b) Count how many months are recorded in that span
...
// 5c) If they requested quarterly/semi-annual but have fewer entries → exit
if ($range !== 'monthly' && $count < $monthsBack) {
    exit(
...
// 5d) Build the human-friendly period label
if ($range === 'monthly') {
    // single-month label
...
foreach ($fields as $f => list($lbl,$rc)) {
    $score   = (int)($row[$f] ?? 0);
    ...
    $pdf->Cell(60,10,strtoupper($lbl),1,0,'L',true);
